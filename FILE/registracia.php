...
if (isset($_POST['email'], $_POST['heslo'], $_POST['mesto'],$_POST['ulica'], $_POST['mobil'])) {
    if ($_POST['email'] != "" && $_POST['heslo'] != "" && $_POST['mesto'] != "" && $_POST['ulica'] != "" && $_POST['mobil'] != "" ){

    if ($storage->ExistEmailCheckRegistrationForm($_POST['email']) == true) {

        echo '<script type="text/javascript"> window.location.href = "prihlasenie.php";alert("Email už existuje!");</script>';

    } else {

        $storage->RegisterUser(new User($_POST['email'], myHash($_POST['email'],$_POST['heslo']), $_POST['mesto'],$_POST['ulica'], $_POST['mobil']));

        echo '<script type="text/javascript">window.location.href = "prihlasenie.php";alert("Registrácia prebehla úspešne!");</script>';
    }
    }else{
        echo '<script type="text/javascript">alert("Všetky polia musia byť vyplnené!");</script>';
    }
}
...
<div class="kontajnerRegister">
    <div class="register">
        ...
        <form method="post" class="formRegister">
            ...
            <input id="mobil" type="tel" placeholder="09XXXXXXXX" name="mobil" value="" pattern="09[0-9]{8}" title="Nemôže obsahovať písmená " required ><br><br>
